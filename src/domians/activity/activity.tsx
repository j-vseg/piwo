...
export function ActivityPage({ id }: { id: string }) {
    ...
    const { data: occurrence, isLoading: isLoadingOccurrence, isError: isErrorOccurrence} = useQuery({
    ...
    const { data: availability, isLoading: isLoadingAvailability, isError: isErrorAvailability } = useQuery({
        ...
        queryFn: user
            ? () => getOccurrenceAvailability(id)
            : skipToken,
    });

    const { data: displayNames, isLoading: isLoadingAccount, isError: isErrorAccounts } = useQuery({
